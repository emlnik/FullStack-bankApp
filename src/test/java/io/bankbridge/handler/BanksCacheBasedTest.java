...
class BanksCacheBasedTest {
    ...
    void handle() throws IOException, InterruptedException {
        ...
        try {
            ...
            request = (HttpRequest) HttpRequest.newBuilder().uri(new URI("http://localhost:8080//v1/banks/all")).GET().build();
            ...
            Gson gson = new Gson();
            List<BankModel> banks1 = gson.fromJson(response.body().toString(), new TypeToken<List<BankModel>>() {
            }.getType());
            assertEquals(20, banks1.size());
